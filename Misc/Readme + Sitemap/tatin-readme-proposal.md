...
Of those 118, 97 fall under "Crawled – currently not indexed". That category means Google fetched the page, read it in full, understood it, and decided it was not worth a place in the index. It is not a crawl failure and not an error.
...
* Not a rendering problem
  ...
  tatin.dev uses no JavaScript. Every page is generated server-side and delivered complete.
* Not a Cloudflare problem

  Googlebot is a verified crawler and receives the full page. I confirmed this through Search Console's live URL test: the package listing renders with all rows present.
* Not a crawl-budget or accessibility problem

  Google crawled all 97 pages. It simply chose not to keep them.
* Not a markup defect

  The server's HTML is sound.
...
The Details page --- for exampke `/v1/packages/details/aplteam-ADOC-7.1.4` --- is the only page type in the registry with content, and what it holds is the package's configuration: a block of key/value pairs (`api`, `date`, `license`, `minimumAplVersion`, `os_lin`, `os_mac`, `os_win`, `source`, `tags`, and so on). Perhaps 150 words, and the vast majority of those words are field *names*, identical on every Details page in the registry.
...
Tatin treats each major version as a distinct package. But within a major line, every published release gets its own Details page. `aplteam-ADOC-7` with a dozen releases produces a dozen Details pages differing only by a version string and a timestamp.

Google indexes one or two of them and discards the rest. Given the registry's "delete none" policy, this multiplication grows over time.
...
**Render each package's README on its package page**

An example is `/v1/packages/versions/<name>-<major>` --- injecting the package's README would turn it from an empty index into the substantive page.
...
Alongside this, one piece of tidying.

At present a package with a dozen releases offers Google a dozen Details pages that are nearly identical, and no indication of which one matters. Google resolves this by picking one --- more or less arbitrarily --- and discarding the rest, which is what the "Crawled – currently not indexed" figure largely consists of.

Search engines provide a standard mechanism for exactly this situation: a single line in a page's HTML header naming a different address as the preferred one for that content. Adding such a line to each Details page, naming the package page, tells Google that the dozen release pages are one package rather than twelve competitors, and that the package page is the one to index.

Nothing is hidden and nothing is removed. Visitors reach every Details page exactly as before, and crawlers may still read them. The only change is that a package's releases stop competing with one another, and whatever standing the package has accumulates on one substantial page, instead of being divided among a dozen thin ones.

### Why this is cheap

* The data already exists

  Every published package supplies a `project_url`. Nothing new is asked of authors.
* The renderer already exists

  MarkAPL converts markdown to HTML5 and is already part of the ecosystem.
* The page structure already exists

  Because Tatin treats each major version as its own package, there is already a natural page per package. No new URL scheme is needed.
* The fetch is deterministic

  Tatin's release tags follow `v{x}.{y}.{z}`, so the README URL is derivable from the version --- no branch detection, no guessing, and each release resolves to the documentation that shipped with it rather than to whatever the repository holds today.
...
* Because the README is fetched from the release's tag, a dormant major line keeps the documentation that was current when it was maintained --- rather than showing its successor's potentially incompatible instructions.
...
* README fetch failures must never block a publish

  Fetching is best-effort; a missing README degrades to today's behaviour.
...
The change is modest, the data and tooling are already in place, and it improves the registry for human visitors whether or not the search results follow.
