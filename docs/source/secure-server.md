...
The `RequestHeader` directive tells Tatin which scheme the outside world used.
Apache adds `X-Forwarded-For`, `X-Forwarded-Host` and `X-Forwarded-Server` of its own accord, but not `X-Forwarded-Proto`, and the hop between Apache and Tatin is plain HTTP even when Apache serves HTTPS.
Without that header Tatin assumes HTTP, and `robots.txt` and `sitemap.xml` then advertise the wrong scheme to search engines.
The directive requires `mod_headers`, which is enabled with `a2enmod headers`.
Writing `%{REQUEST_SCHEME}` rather than a literal `https` keeps it honest in a vhost that serves plain HTTP as well.

Rather than the header you may set `[CONFIG]BaseURL` in Plodder's INI file to the public URL, for example `'https://tatin.dev'`.
That takes precedence over anything derived from the request, and it is the only reliable option when the registry is served under a path prefix.

Either way `[CONFIG]Secure` stays `0`: it describes the connection Tatin itself accepts, and that one is not encrypted.
